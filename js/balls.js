...
function throwPotions(b,other,g,count){
  ...
  // Return by Death: a dodged throw lands nothing at all (the self-heal potion included)
  for(const type of (skillDodge(other,g)?[]:chosen)){
...
{id:'blade', name:'Blade Ball', group:'dps', icon:'🗡️', hp:75, speed:100, dmg:10, color:'#c9c9d6',
 descSimple:'Những thanh kiếm xoay quanh thân, số kiếm tăng dần theo thời gian sống, gây sát thương khi địch đến gần.',
 desc:'Những thanh kiếm thật (có chuôi, cán, chắn kiếm, lưỡi thép với rãnh máu và mũi nhọn) quay quanh thân với phạm vi rộng. Bắt đầu trận với sẵn 1 thanh kiếm, số lượng tăng dần theo thời gian sống (tối đa 5), gây sát thương liên tục khi địch áp sát.',
 ...
 renderExtra:(b,g,ctx)=>{
   ...
   const R=b.radius;
   const spin=g.t*3;
   const hitFlash=(g.t-(b.state.bladeTickCD||-99))<0.15;   // swords flare when they connect
   // Sword layout, in local space where +x = "outward from the ball":
   //   pommel -> grip -> crossguard -> blade (with fuller) -> pointed tip.
   const X0=R+9;            // pommel centre
   const GRIP0=X0+3, GRIP1=X0+14;
   const GUARD0=GRIP1, GUARD1=GRIP1+3.6;
   const B0=GUARD1, BL=38;  // blade start / length -> tip lands near the damage range
   const TIP=B0+BL;

   // steel gradient across the blade width: bright edges, darker centre ridge
   const steel=ctx.createLinearGradient(0,-4.6,0,4.6);
   steel.addColorStop(0,'#f8fbff');
   steel.addColorStop(0.42,'#c5cedd');
   steel.addColorStop(0.5,'#7f8b9f');
   steel.addColorStop(0.58,'#d9e0ec');
   steel.addColorStop(1,'#98a4b8');

   // 1) fading crescent sweep trailing behind the spinning swords
   ctx.save();
   ctx.lineCap='round';
   for(let i=0;i<n;i++){
     ...
     for(let k=1;k<=6;k++){
       ctx.beginPath();
       ctx.arc(b.x,b.y,TIP-6,a0-k*0.11,a0-(k-1)*0.11);
       ctx.strokeStyle=`rgba(200,225,255,${((1-k/7)*(hitFlash?0.5:0.3)).toFixed(3)})`;
       ctx.lineWidth=5-k*0.55;
       ctx.stroke();
     }
   }
   ctx.restore();

   // 2) the swords themselves
   for(let i=0;i<n;i++){
     const ang=spin+(i/n)*Math.PI*2;
     ctx.save();
     ctx.translate(b.x,b.y); ctx.rotate(ang);
     ctx.lineJoin='round'; ctx.lineCap='round';
     ctx.shadowColor=hitFlash?'#fff3b0':'#9fd0ff';
     ctx.shadowBlur=hitFlash?14:6;

     // blade
     ctx.beginPath();
     ctx.moveTo(B0,-4.4);
     ctx.lineTo(B0+BL*0.8,-3.5);
     ctx.lineTo(TIP,0);
     ctx.lineTo(B0+BL*0.8,3.5);
     ctx.lineTo(B0,4.4);
     ctx.closePath();
     ...
     ctx.shadowBlur=0;
     ctx.lineWidth=1; ctx.strokeStyle='#39424f'; ctx.stroke();
     // fuller (blood groove) down the middle of the blade
     ctx.beginPath(); ctx.moveTo(B0+3,0); ctx.lineTo(B0+BL*0.82,0);
     ctx.strokeStyle='rgba(60,70,88,0.75)'; ctx.lineWidth=1.3; ctx.stroke();
     // bright leading edge
     ctx.beginPath(); ctx.moveTo(B0+1,-4.0); ctx.lineTo(B0+BL*0.8,-3.1); ctx.lineTo(TIP-1.5,-0.3);
     ctx.strokeStyle='rgba(255,255,255,0.9)'; ctx.lineWidth=0.9; ctx.stroke();

     // crossguard: gold bar with slightly upswept, rounded ends
     ctx.beginPath();
     ctx.moveTo(GUARD0,-3);
     ctx.lineTo(GUARD0-1.4,-10.5);
     ctx.quadraticCurveTo(GUARD0+1.8,-12.2,GUARD1+0.6,-9.6);
     ctx.lineTo(GUARD1,-3);
     ctx.lineTo(GUARD1,3);
     ctx.lineTo(GUARD1+0.6,9.6);
     ctx.quadraticCurveTo(GUARD0+1.8,12.2,GUARD0-1.4,10.5);
     ctx.lineTo(GUARD0,3);
     ctx.closePath();
     const gold=ctx.createLinearGradient(GUARD0,0,GUARD1,0);
     gold.addColorStop(0,'#f7d774'); gold.addColorStop(1,'#a8781c');
     ctx.fillStyle=gold; ctx.fill();
     ctx.lineWidth=0.9; ctx.strokeStyle='#5c3f0c'; ctx.stroke();

     // grip: dark leather with diagonal wraps
     ctx.fillStyle='#4a2e1a';
     ctx.fillRect(GRIP0,-2.3,GRIP1-GRIP0,4.6);
     ctx.strokeStyle='rgba(20,10,4,0.75)'; ctx.lineWidth=0.9;
     for(let x=GRIP0+1.5;x<GRIP1-0.5;x+=2.6){
       ctx.beginPath(); ctx.moveTo(x,-2.3); ctx.lineTo(x+1.6,2.3); ctx.stroke();
     }

     // pommel: gold ball with a small ruby
     ctx.beginPath(); ctx.arc(X0,0,3.4,0,Math.PI*2);
     ctx.fillStyle=gold; ctx.fill();
     ctx.lineWidth=0.9; ctx.strokeStyle='#5c3f0c'; ctx.stroke();
     ctx.beginPath(); ctx.arc(X0-0.2,0,1.2,0,Math.PI*2);
     ctx.fillStyle='#e0364b'; ctx.fill();

     // a twinkle that runs along the tip now and then
     const tw=Math.max(0,Math.sin(g.t*4+i*1.7));
     if(tw>0.85){
       const a=(tw-0.85)/0.15;
       ctx.globalAlpha=a;
       ctx.strokeStyle='#ffffff'; ctx.lineWidth=1.2;
       ctx.beginPath(); ctx.moveTo(TIP-3.5,-3.5); ctx.lineTo(TIP-3.5,3.5);
       ctx.moveTo(TIP-7,0); ctx.lineTo(TIP,0); ctx.stroke();
     }
     ctx.restore();
   }
 }},
...
{id:'charge', name:'Charge Ball', group:'burst', icon:'🔋', hp:95, speed:85, dmg:8, color:'#f0c419',
 ...
 onBallCollide:(b,other,g)=>{
   ...
   if(charge>=2.5 && !skillDodge(other,g)){
...
{id:'snake', name:'Snake Ball', group:'dps', icon:'🐍', hp:90, speed:110, dmg:10, color:'#7fbf3f',
 descSimple:'Rắn đuôi to bản, thỉnh thoảng lè lưỡi. Ăn táo mọc trên sân để dài thêm 1 đốt; đuôi quật trúng đối thủ thì gây sát thương & hất lùi nhưng mất 1 đốt. Đuôi càng dài đòn càng đau.',
 desc:'Cứ mỗi lúc lại có 2 quả táo mọc ngẫu nhiên trên sân (chỉ Snake ăn được): +1 đốt/quả, tối đa 20 đốt. Đuôi quét trúng đối thủ gây sát thương (tăng theo số đốt) và hất lùi, đổi lại mất 1 đốt (hồi 0.7s; đuôi dưới 4 đốt thì chưa quật được). Rắn tự uốn nhẹ về phía táo ở gần.',
 init:(b)=>{ snakeInit(b); },
 update:(b,dt,g)=>{ snakeUpdate(b,dt,g); },
 // custom body: apples + wide segmented tail + a real snake face with a tongue
 drawBody:(b,g,ctx,info)=>{ snakeDraw(b,g,ctx,info); }},
 ...
 update:(b,dt,g)=>{
   ...
   if(other){
     ...
     if(distToTip<other.radius+10 || distToRope<other.radius+7){
       if(skillDodge(other,g)){ b.state.hook=null; return; }
 ...
 update:(b,dt,g)=>{
   ...
   for(const anchor of b.state.webAnchors){
     ...
     if(d<other.radius+5 && g.t-anchor.lastHit>0.3){
       anchor.lastHit=g.t;
       if(skillDodge(other,g)) continue;
...
{id:'reforge', name:'Reforge Ball', group:'dps', icon:'⚒️', hp:100, speed:100, dmg:8, color:'#b5652f',
 descSimple:'Va tường sẽ triệu hồi một linh hồn nhỏ tự bắn đạn hỗ trợ; linh hồn tích lực khi đánh trúng địch — càng tích nhiều, đạn càng mạnh VÀ bắn càng nhanh; có thêm một linh hồn nữa khi máu xuống thấp.',
 desc:'Đập vào tường để triệu hồi tiểu linh hồn bắn đạn hỗ trợ, tích lũy sức mạnh nhanh mỗi khi đánh trúng địch. Càng tích lực, sát thương mỗi viên đạn lẫn tốc độ bắn đều tăng (từ 1 phát mỗi 2.0 giây ở mức thấp nhất lên 1 phát mỗi 0.9 giây ở mức tối đa, đạn bay nhanh hơn một chút). Khi HP dưới 50%, triệu hồi thêm linh hồn thứ hai.',
 ...
 update:(b,dt,g)=>{
   ...
   for(const m of b.state.minions){
     ...
     // charge-up: the more power the minion has stored, the FASTER it fires
     // (2.0s between shots at power 1 -> 0.9s at max power 10) AND the harder
     // and quicker each shot hits
     const heat=clamp((m.power-1)/9,0,1);
     const fireInterval=2.0-1.1*heat;
     if(g.t-(m.fireCD||0)>fireInterval){
       m.fireCD=g.t;
       if(other){
         const ang=Math.atan2(other.y-m.y,other.x-m.x);
         const shotSpd=310+50*heat;
         fireProjectile(g,b,{x:m.x,y:m.y,vx:Math.cos(ang)*shotSpd,vy:Math.sin(ang)*shotSpd,r:5,dmg:dmgFor(b,2)*m.power*0.45,life:1.5,color:'#c9a8ff'});
       }
     }
   }
 },
...
{id:'conductor', name:'Train Ball', group:'burst', icon:'🚂', hp:95, speed:100, dmg:13, color:'#c97a2f',
 descSimple:'Va tường đánh dấu điểm ray; cứ 4 lần va tường một đoàn tàu chạy dọc ray đó, gây sát thương nặng và hất văng ai cản đường.',
 desc:'Va tường để lại điểm ray nối với điểm chạm trước đó. Cứ mỗi 4 lần va tường, một đoàn tàu (hitbox lớn) chạy đúng theo đường ray đã tạo, gây sát thương cố định 22 và húc văng/hất tung bất kỳ ai cản đường. Chạy xong, đường ray đó biến mất và phải tạo lại từ đầu.',
 init:(b)=>{ b.state.wallHits=0; b.state.railPoints=[]; b.state.activeRail=null; },
 // every 4th wall hit launches a train along the saved rail points (no timer)
 onWallHit:(b,g)=>{
   b.state.railPoints.push({x:b.x,y:b.y});
   if(b.state.railPoints.length>6) b.state.railPoints.shift();
   b.state.wallHits=(b.state.wallHits||0)+1;
   if(b.state.wallHits<4) return;
   b.state.wallHits=0; // next cycle starts from the following wall hit
   ...
   // when it finishes, drop only THIS train's rail: wall hits recorded while it
   // was running belong to the next train (and a newer train may already be active)
   g.spawnTrain(b,path,()=>{
     if(b.state.activeRail===path) b.state.activeRail=null;
     b.state.railPoints=b.state.railPoints.filter(p=>!path.includes(p));
   });
...
{id:'werewolf', name:'Werewolf Ball', group:'special', icon:'🐺', hp:30, speed:95, dmg:0, color:'#e8b48c',
 descSimple:'Vào trận yếu ớt ở dạng người (có khiên miễn thương 1 lần). Sau 13 giây hoá sói trong 10 giây: máu và tốc độ tăng vọt, tự lao vào đối thủ và cào sát thương liên tục, rồi hoá lại người với máu đầy.',
 desc:'Bắt đầu trận ở dạng người: HP nền rất thấp (30), không gây sát thương khi va chạm. Sau 13 giây ở dạng người, môi trường tối lại như ban đêm với một vầng trăng tròn hiện giữa map và ball hoá thành Werewolf: máu tối đa tăng 400% (gấp 5 so với mốc người), tốc độ +50%, tự hồi đầy máu rồi hồi thêm 4 HP mỗi giây, cứ mỗi giây tự đổi hướng lao thẳng về phía đối thủ, và có một vùng cào (vòng nguy hiểm) quanh thân — hễ đối thủ lọt vào vùng này sẽ liên tục lãnh 9 sát thương mỗi 0.2 giây kèm hiệu ứng cào/chém đầy đủ. Dạng sói duy trì 10 giây rồi trời sáng trở lại, ball hoá về người với máu đầy theo đúng mốc máu tối đa gốc, không bị trừ. Ngoài ra, CHỈ Ở DẠNG NGƯỜI ball mới có khiên miễn nhiễm hoàn toàn đòn đánh kế tiếp (có sẵn ngay khi vào trận và được nạp lại mỗi lần hoá từ sói về người; dùng 1 lần rồi mất). Dạng sói không có khiên miễn thương. Các mốc 13 giây (người) và 10 giây (sói) chạy theo thời gian trận đấu: chỉ bắt đầu đếm khi giao tranh thực sự bắt đầu (không tính lúc hai bên đang ngắm) và đứng yên hoàn toàn khi Clock Ball ngưng đọng thời gian.',
 init:(b)=>{
   b.state.form='human';
   // NOT 0: game time already runs during the aiming phases, so anchoring the
   // human-form timer to t=0 made the first transformation arrive early (by
 ...
 // HUMAN FORM ONLY: any hit that would land while the charge is up is fully
 // negated instead - given at match start and again each time the ball turns
 // back into a human. The wolf form never has it.
 modifyIncoming:(b,attacker,dmg,g,source)=>{
   if(b.state.form==='human' && b.state.shieldReady && dmg>0){
     b.state.shieldReady=false;
 ...
 update:(b,dt,g)=>{
   ...
   if(b.state.form==='human' && elapsed>=WEREWOLF_HUMAN_SECONDS){
     b.state.form='wolf';
     b.state.formSince=g.t;
     b.state.nightMode=true;
     b.state.shieldReady=false; // wolf form has NO damage-immunity shield
   ...
   } else if(b.state.form==='wolf' && elapsed>=WEREWOLF_WOLF_SECONDS){
     b.state.form='human';
     b.state.formSince=g.t;
     b.state.nightMode=false;
     b.state.shieldReady=true; // fresh immunity charge, human form only
   ...
   if(dist(b.x,b.y,other.x,other.y)<range && g.t>=(b.state.clawCD||0)){
     ...
     if(!skillDodge(other,g)){
       other.hp=Math.max(0,other.hp-9);
       spawnFloatText(g,other.x,other.y-30,'-9','#ef4444');
 ...
 renderExtra:(b,g,ctx)=>{
   ...
   // shield-charge indicator: a thin pulsing ring worn in HUMAN form
   ...
   if(b.state.form==='human' && b.state.shieldReady){
     ctx.save();
...
{id:'clock', name:'Clock Ball', group:'special', icon:'⏰', hp:95, speed:100, dmg:8, color:'#c9a227', collidePriority:1,
 ...
 onBallCollide:(b,other,g)=>{
   ...
   if(skillDodge(other,g)) return; // Return by Death dodged the time-stop
...
{id:'returnbydeath', name:'Return by Death Ball', group:'special', icon:'🔁', hp:70, speed:70, dmg:4, color:'#7b5ea7',
 descSimple:'Chết rồi bấm Đấu lại (cùng bóng, cùng đối thủ) sẽ nhận thêm 1 tầng: mạnh hơn, trâu hơn, nhanh hơn, giảm sát thương và có cơ hội né kỹ năng đặc biệt của đối thủ. Về Menu là mất hết tầng.',
 desc:'Không có kỹ năng chủ động. Mỗi lần Return by Death chết rồi người chơi chọn ĐẤU LẠI với đúng đối thủ đó trong cùng một chuỗi trận liên tiếp, bóng nhận thêm 1 tầng Return by Death (cộng dồn, tính từ chỉ số gốc): sát thương x1.15, máu tối đa x1.15, tốc độ x1.15, giảm sát thương nhận vào x1.05 (mỗi tầng) và +3 điểm phần trăm cơ hội NÉ kỹ năng đặc biệt của đối thủ (tầng 3 = 9%). Né thành công sẽ bỏ hoàn toàn cú kỹ năng đó (không sát thương, không hiệu ứng), hiện chữ DODGE!; va chạm vật lý thông thường không né được. Tầng chỉ tồn tại trong chuỗi chơi hiện tại: bấm Menu chính hoặc đổi bóng/đối thủ là về 0.',
 // Stacks come from main.js (returnDeathRuns) through the Ball constructor. init runs
 // exactly once on a freshly built Ball whose stats are still the unbuffed base, and
 // every multiplier is computed from the stack count with Math.pow (never x1.15 on top
 // of an already-buffed value).
 init:(b)=>{
   const s=b.returnStacks||0;
   const up=Math.pow(1.15,s);
   b.maxHp*=up; b.hp=b.maxHp;
   b.dmg*=up;
   b.baseSpeed*=up;
   b.state.dmgResist=Math.pow(1.05,s); // damage taken is divided by this (see GameEngine._postStep)
   b.state.dodgeChance=0.03*s;         // +3 percentage points per stack (see skillDodge)
 }},
];
...
// ---- Snake Ball ------------------------------------------------------------
// Length = number of tail segments ("đốt"). The ONLY way to grow is eating apples
// (2 spawn at a time, random spots, only the snake can eat them). A tail lash that
// lands costs 1 segment - so every hit is paid for with an apple.
const SNAKE={
  START:6, MIN:3, MAX:20,       // tail length in segments
  MIN_LASH:4,                   // the tail needs at least this many segments to lash
  W_NECK:0.74, W_TIP:0.36,      // tail half-width as a fraction of the ball radius (wide at the neck, tapering)
  SPACING:0.92,                 // distance between segment centres = this x the segment's half-width
  LASH_CD:0.7,                  // seconds between two lashes
  LASH_BASE:4, LASH_PER_SEG:0.25, // lash damage = dmgFor(b, BASE + PER_SEG * length)
  LASH_KNOCK:210,               // shove on the victim (a decaying impulse, never a permanent speed-up)
  LASH_SKIP:2,                  // segments right behind the head never lash (the head hit is the normal collision)
  SWING_DUR:0.4, SWING_CD:2.2,  // tail whip: it sweeps around the head when the opponent is inside tail reach
  SWING_OVERSHOOT:0.9, SWING_LAG:0.5, // sweep goes this many rad past the target; the tip trails the base by this much
  APPLE_R:10.5, EAT_PAD:14,        // apple drawn radius / extra reach on top of the head radius
  APPLE_LIFE:12,                // seconds an uneaten apple stays before it rots away
  APPLE_FIRST:1.0,              // first pair appears this many seconds into the fight
  APPLE_GAP_MIN:4.5, APPLE_GAP_MAX:7, // random pause between two pairs
  APPLE_CAP:4,                  // never more than this many apples on the map at once
  SENSE:230, SENSE_ARC:1.9, TURN:2.2, // apple sense: range (px), widest bend it will try (rad), turn speed (rad/s)
};

function snakeInit(b){
  b.state.snk={
    segs:SNAKE.START, nodes:[], apples:[],
    appleTimer:SNAKE.APPLE_FIRST, lashCD:0.5,
    face:b.player===2?Math.PI:0, fxLast:null,
    tongueAt:-9, tongueFlicks:1, tongueNext:null, blinkSeed:rand(0,3),
    eatTimes:[], chompAt:-9, lashAt:-9, lashIdx:0, drops:[],
    swing:null, swingCD:1.5, swingTrail:null,
    eaten:0, lashes:0, spawned:0            // bookkeeping only (used by the balance simulator)
  };
}

// half-width of tail segment k out of n (k=0 is the one next to the head)
function snakeNodeR(b,k,n){
  const u=n>1?k/(n-1):0;
  return b.radius*(SNAKE.W_NECK+(SNAKE.W_TIP-SNAKE.W_NECK)*u);
}

// Rope-style chain: every segment trails the one in front of it. Idempotent, so it is
// safe to call from update() AND from the renderer (the tail must keep following the
// head even while the snake is powerless and update() is not running).
function snakeFollow(b,g){
  const s=b.state.snk; if(!s) return;
  const T=g.fxT||0;
  const dt=s.fxLast==null?0:clamp(T-s.fxLast,0,0.05);
  s.fxLast=T;
  if(Math.hypot(b.vx,b.vy)>25) s.face=angleLerp(s.face,Math.atan2(b.vy,b.vx),1-Math.exp(-16*dt));
  // tongue + blink schedule (effect clock, so it also idles during the aiming phase)
  if(s.tongueNext==null) s.tongueNext=T+rand(1.2,3);
  if(T>=s.tongueNext){
    s.tongueAt=T; s.tongueFlicks=Math.random()<0.35?2:1;
    s.tongueNext=T+0.34*s.tongueFlicks+rand(2.2,4.6);
  }
  const n=s.segs, nodes=s.nodes;
  while(nodes.length<n){                       // build / grow: continue the line behind the last segment
    const k=nodes.length;
    const a=k>0?nodes[k-1]:{x:b.x,y:b.y};
    let dx=-Math.cos(s.face), dy=-Math.sin(s.face);
    if(k>=2){ dx=a.x-nodes[k-2].x; dy=a.y-nodes[k-2].y; const d=Math.hypot(dx,dy)||1; dx/=d; dy/=d; }
    const sp=snakeNodeR(b,k,n)*SNAKE.SPACING;
    nodes.push({x:a.x+dx*sp,y:a.y+dy*sp});
  }
  if(nodes.length>n) nodes.length=n;
  const sw=s.swing;
  if(sw){                                       // whip: every segment sweeps round the head, the tip lagging behind the base
    let qx=b.x, qy=b.y;
    for(let k=0;k<n;k++){
      const r=snakeNodeR(b,k,n), sp=r*SNAKE.SPACING;
      const q=clamp(sw.p*(1+SNAKE.SWING_LAG)-k/n*SNAKE.SWING_LAG,0,1);
      const e=q<0.5?2*q*q:1-Math.pow(-2*q+2,2)/2;
      const ang=sw.a0+sw.dir*sw.arc*e;
      qx+=Math.cos(ang)*sp; qy+=Math.sin(ang)*sp;
      nodes[k].x=clamp(qx,r,g.w-r); nodes[k].y=clamp(qy,r,g.h-r);
    }
    return;
  }
  let px=b.x, py=b.y;
  for(let k=0;k<n;k++){
    const nd=nodes[k], r=snakeNodeR(b,k,n), sp=r*SNAKE.SPACING;
    let dx=nd.x-px, dy=nd.y-py, d=Math.hypot(dx,dy);
    if(d<1e-4){ dx=-Math.cos(s.face); dy=-Math.sin(s.face); d=1; }
    const tgt=d>sp?sp:(d<sp*0.55?sp*0.55:d);   // trails like a rope: pulled when stretched, only pushed when crushed
    nd.x=clamp(px+dx/d*tgt,r,g.w-r); nd.y=clamp(py+dy/d*tgt,r,g.h-r);
    px=nd.x; py=nd.y;
  }
}

// true if (x,y) is inside / next to a wall, a wall's whole sliding lane, or a portal
function snakeBlocked(g,x,y,pad){
  for(const o of g.mapObstacles){
    let x0,x1,y0,y1;
    if(o.type==='circle'){ x0=o.x-o.r; x1=o.x+o.r; y0=o.y-o.r; y1=o.y+o.r; }
    else { x0=o.x; x1=o.x+o.w; y0=o.y; y1=o.y+o.h; }
    if(o.mover){                                // sliding wall: block its entire track
      const m=o.mover, cur=o[m.axis];
      if(m.axis==='x'){ x0-=cur-(m.base-m.amp); x1+=(m.base+m.amp)-cur; }
      else { y0-=cur-(m.base-m.amp); y1+=(m.base+m.amp)-cur; }
    }
    if(x>x0-pad && x<x1+pad && y>y0-pad && y<y1+pad) return true;
  }
  for(const p of (g.mapPortals||[])){
    if(dist(x,y,p.a.x,p.a.y)<p.a.r+pad+6 || dist(x,y,p.b.x,p.b.y)<p.b.r+pad+6) return true;
  }
  return false;
}

function snakeApplePoint(g,b,s,other,avoid){
  const M=34;
  for(let tries=0;tries<40;tries++){
    const x=rand(M,g.w-M), y=rand(M,g.h-M);
    if(dist(x,y,b.x,b.y)<b.radius+70) continue;
    if(other && dist(x,y,other.x,other.y)<other.radius+40) continue;
    if(avoid.some(p=>dist(x,y,p.x,p.y)<170)) continue;   // the two apples of a pair land far apart
    if(s.apples.some(p=>dist(x,y,p.x,p.y)<90)) continue;
    if(snakeBlocked(g,x,y,22)) continue;
    return {x,y};
  }
  return null;
}

// exactly TWO apples per drop
function snakeSpawnApples(b,g,s){
  const other=g.opponentOf(b), placed=[];
  for(let i=0;i<2;i++){
    const p=snakeApplePoint(g,b,s,other,placed);
    if(!p) continue;
    placed.push(p);
    s.apples.push({x:p.x,y:p.y,age:0,seed:rand(0,6.28)});
    s.spawned++;
    spawnParticles(g,p.x,p.y,7,{color:'#ff6b6b',type:'glow',speed:70,life:0.4});
  }
}

// how far the outstretched tail reaches from the head centre
function snakeReach(b,s){
  let L=0; const n=s.segs;
  for(let k=0;k<n;k++) L+=snakeNodeR(b,k,n)*SNAKE.SPACING;
  return L;
}
function snakeStartSwing(b,g,s,other){
  const tip=s.nodes[s.nodes.length-1];
  const a0=Math.atan2(tip.y-b.y,tip.x-b.x);
  const beta=Math.atan2(other.y+other.vy*0.16-b.y,other.x+other.vx*0.16-b.x);   // aim a touch ahead of the target
  let d=beta-a0; d=((d+Math.PI)%(Math.PI*2)+Math.PI*2)%(Math.PI*2)-Math.PI;
  s.swing={p:0,a0,dir:d>=0?1:-1,arc:Math.abs(d)+SNAKE.SWING_OVERSHOOT,trail:[]};
  s.swingCD=SNAKE.SWING_CD;
}

function snakeUpdate(b,dt,g){
  const s=b.state.snk; if(!s) return;
  const T=g.fxT||0;
  if(s.swing){
    s.swing.p+=dt/SNAKE.SWING_DUR;
    if(s.swing.p>=1){ s.swingTrail={pts:s.swing.trail,end:T}; s.swing=null; }
  }
  snakeFollow(b,g);
  if(s.swing){ const tp=s.nodes[s.nodes.length-1]; s.swing.trail.push({x:tp.x,y:tp.y}); if(s.swing.trail.length>9) s.swing.trail.shift(); }
  if(s.swingCD>0) s.swingCD-=dt;

  // ---- apples: age, rot away, drop a new pair every so often ----
  for(const a of s.apples) a.age+=dt;
  s.apples=s.apples.filter(a=>a.age<SNAKE.APPLE_LIFE);
  s.appleTimer-=dt;
  if(s.appleTimer<=0){
    if(s.apples.length<=SNAKE.APPLE_CAP-2) snakeSpawnApples(b,g,s);
    s.appleTimer=rand(SNAKE.APPLE_GAP_MIN,SNAKE.APPLE_GAP_MAX);
  }

  // ---- eat (+1 segment per apple; a full-length snake just leaves them) ----
  if(s.segs<SNAKE.MAX){
    for(let i=s.apples.length-1;i>=0;i--){
      const a=s.apples[i];
      if(a.age<0.25 || dist(b.x,b.y,a.x,a.y)>=b.radius+SNAKE.EAT_PAD) continue;
      s.apples.splice(i,1);
      s.segs=Math.min(SNAKE.MAX,s.segs+1); s.eaten++;
      s.eatTimes.push(T); s.chompAt=T; s.tongueAt=T; s.tongueFlicks=1; s.tongueNext=Math.max(s.tongueNext||0,T+2);
      spawnFloatText(g,b.x,b.y-b.radius-14,'+1 đốt','#2f9e44');
      spawnParticles(g,a.x,a.y,10,{color:'#ff6b6b',type:'glow',speed:130,life:0.35});
      spawnParticles(g,a.x,a.y,5,{color:'#8fdc4a',type:'spark',speed:150,life:0.3});
      if(s.segs>=SNAKE.MAX) break;
    }
  }

  // ---- apple sense: bend gently toward the nearest apple that is close and roughly ahead ----
  if(s.segs<SNAKE.MAX && dt>0){
    let best=null, bd=SNAKE.SENSE;
    for(const a of s.apples){ if(a.age<0.25) continue; const d=dist(b.x,b.y,a.x,a.y); if(d<bd){ bd=d; best=a; } }
    if(best && Math.hypot(b.vx,b.vy)>1){
      let df=Math.atan2(best.y-b.y,best.x-b.x)-Math.atan2(b.vy,b.vx);
      df=((df+Math.PI)%(Math.PI*2)+Math.PI*2)%(Math.PI*2)-Math.PI;
      if(Math.abs(df)<SNAKE.SENSE_ARC){
        const rot=clamp(df,-SNAKE.TURN*dt,SNAKE.TURN*dt), c=Math.cos(rot), sn=Math.sin(rot);
        const vx=b.vx*c-b.vy*sn, vy=b.vx*sn+b.vy*c;
        b.vx=vx; b.vy=vy;                          // direction only - the locked speed is untouched
      }
    }
  }

  // ---- tail lash: the wide tail sweeps the opponent -> damage + shove, costs 1 segment ----
  if(s.lashCD>0) s.lashCD-=dt;
  const other=g.opponentOf(b);
  if(other && other.alive && !s.swing && s.swingCD<=0 && s.segs>=SNAKE.MIN_LASH
     && dist(b.x,b.y,other.x,other.y)<snakeReach(b,s)+other.radius*0.5) snakeStartSwing(b,g,s,other);
  if(other && other.alive && s.lashCD<=0 && s.segs>=SNAKE.MIN_LASH){
    const n=s.nodes.length;
    for(let k=SNAKE.LASH_SKIP;k<n;k++){
      const nd=s.nodes[k];
      if(dist(nd.x,nd.y,other.x,other.y)>=other.radius+snakeNodeR(b,k,n)-4) continue;
      s.lashCD=SNAKE.LASH_CD; s.lashAt=T; s.lashIdx=k; s.lashes++;
      const dmg=dmgFor(b,SNAKE.LASH_BASE+SNAKE.LASH_PER_SEG*s.segs);
      g.applyProjectileDamage({owner:b,dmg,x:nd.x,y:nd.y,color:'#3bb54a'},other);
      if(!other.state.ccImmune) applyImpulse(other,Math.atan2(other.y-nd.y,other.x-nd.x),SNAKE.LASH_KNOCK);
      // pay with the tip segment: it snaps off and fades
      const tip=s.nodes[n-1];
      s.drops.push({x:tip.x,y:tip.y,r:snakeNodeR(b,n-1,n),born:T,ang:Math.atan2(tip.y-s.nodes[Math.max(0,n-2)].y,tip.x-s.nodes[Math.max(0,n-2)].x)});
      s.segs=Math.max(SNAKE.MIN,s.segs-1);
      spawnFloatText(g,tip.x,tip.y-14,'-1 đốt','#5a8f1e');
      spawnParticles(g,nd.x,nd.y,8,{color:'#c8ff9a',type:'spark',speed:230,life:0.3});
      g.triggerShake(2.6,0.1);
      break;
    }
  }
}
...
/* ---------------------------------- drawing ---------------------------------- */
function snakeDrawApple(ctx,g,a){
  const T=g.fxT, k=easeOutBack(a.age/0.35);
  if(k<=0.02) return;
  const left=SNAKE.APPLE_LIFE-a.age;
  const alpha=left<2.5?((Math.floor(T*(left<1?12:6))%2===0)?1:0.4):1;   // blink shortly before it rots
  const r=SNAKE.APPLE_R*k, bob=Math.sin(T*3+a.seed)*1.6;
  ctx.save();
  ctx.globalAlpha=alpha;
  ctx.fillStyle='rgba(0,0,0,0.16)';
  ctx.beginPath(); ctx.ellipse(a.x,a.y+r*1.05,r*0.9,r*0.32,0,0,Math.PI*2); ctx.fill();
  ctx.translate(a.x,a.y+bob);
  ctx.shadowColor='rgba(255,70,70,0.6)'; ctx.shadowBlur=10;
  ctx.beginPath();
  ctx.arc(-r*0.3,-r*0.05,r*0.78,0,Math.PI*2); ctx.moveTo(r*0.3+r*0.78,-r*0.05);
  ctx.arc(r*0.3,-r*0.05,r*0.78,0,Math.PI*2); ctx.moveTo(r*0.8,r*0.18);
  ctx.arc(0,r*0.18,r*0.8,0,Math.PI*2);
  const gr=ctx.createRadialGradient(-r*0.35,-r*0.4,r*0.1,0,0,r*1.2);
  gr.addColorStop(0,'#ff9a8f'); gr.addColorStop(0.45,'#e53535'); gr.addColorStop(1,'#a01818');
  ctx.fillStyle=gr; ctx.fill();
  ctx.shadowBlur=0;
  ctx.lineWidth=1.4; ctx.strokeStyle='#5b0f0f'; ctx.stroke();
  ctx.fillStyle='rgba(255,255,255,0.65)';
  ctx.beginPath(); ctx.ellipse(-r*0.42,-r*0.32,r*0.2,r*0.12,-0.7,0,Math.PI*2); ctx.fill();
  ctx.strokeStyle='#6b4423'; ctx.lineWidth=Math.max(1.5,r*0.2); ctx.lineCap='round';
  ctx.beginPath(); ctx.moveTo(0,-r*0.5); ctx.quadraticCurveTo(r*0.05,-r*0.85,r*0.2,-r*1.0); ctx.stroke();
  ctx.fillStyle='#4caf3a'; ctx.strokeStyle='#2c6b20'; ctx.lineWidth=1;
  ctx.beginPath(); ctx.ellipse(r*0.55,-r*0.95,r*0.45,r*0.22,-0.5,0,Math.PI*2); ctx.fill(); ctx.stroke();
  ctx.restore();
}

function snakeDrawTail(b,g,ctx){
  const s=b.state.snk, nodes=s.nodes, n=nodes.length;
  if(!n) return;
  const T=g.fxT, base=b.def.color;
  // whip trail: a fading streak behind the tail tip while it sweeps
  const tr=s.swing?s.swing.trail:(s.swingTrail&&T-s.swingTrail.end<0.22?s.swingTrail.pts:null);
  if(tr && tr.length>1){
    const fade=s.swing?1:1-(T-s.swingTrail.end)/0.22;
    ctx.save(); ctx.lineCap='round';
    for(let i=1;i<tr.length;i++){
      const u=i/tr.length;
      ctx.strokeStyle='rgba(215,255,170,'+(0.55*u*fade).toFixed(2)+')'; ctx.lineWidth=2+7*u;
      ctx.beginPath(); ctx.moveTo(tr[i-1].x,tr[i-1].y); ctx.lineTo(tr[i].x,tr[i].y); ctx.stroke();
    }
    ctx.restore();
  }
  s.eatTimes=s.eatTimes.filter(t=>T-t<1.6);
  const rads=[];
  for(let k=0;k<n;k++){
    let bulge=0;
    for(const t of s.eatTimes){ const p=(T-t)/0.05; bulge+=Math.max(0,1-Math.abs(k-p)/2.4)*0.32; }   // a lump travels down the tail after eating
    rads.push(snakeNodeR(b,k,n)*(1+bulge));
  }
  ctx.save();
  // soft ground shadow
  ctx.fillStyle='rgba(0,0,0,0.13)';
  for(let k=n-1;k>=0;k--){ ctx.beginPath(); ctx.ellipse(nodes[k].x,nodes[k].y+rads[k]*0.6,rads[k]*0.95,rads[k]*0.4,0,0,Math.PI*2); ctx.fill(); }
  // owner-colour rim around the whole silhouette (same cue the ball outline gives)
  ctx.fillStyle=b.color;
  for(let k=n-1;k>=0;k--){ ctx.beginPath(); ctx.arc(nodes[k].x,nodes[k].y,rads[k]+2.4,0,Math.PI*2); ctx.fill(); }
  // segments, tip first so the ones nearer the head overlap the ones behind them
  const wave=(T-s.lashAt)<0.5?s.lashIdx+(T-s.lashAt)/0.025:-99;                    // whip-crack flash running to the tip
  for(let k=n-1;k>=0;k--){
    const nd=nodes[k], r=rads[k];
    const prev=k>0?nodes[k-1]:{x:b.x,y:b.y};
    const ang=Math.atan2(prev.y-nd.y,prev.x-nd.x);
    const col=(k%2===0)?base:darken(base,0.12);
    const gr=ctx.createRadialGradient(nd.x-r*0.35,nd.y-r*0.4,r*0.1,nd.x,nd.y,r*1.05);
    gr.addColorStop(0,lighten(col,0.5)); gr.addColorStop(0.55,col); gr.addColorStop(1,darken(col,0.25));
    ctx.beginPath(); ctx.arc(nd.x,nd.y,r,0,Math.PI*2);
    ctx.fillStyle=gr; ctx.fill();
    ctx.lineWidth=1.3; ctx.strokeStyle='rgba(28,52,14,0.7)'; ctx.stroke();
    // dorsal diamond
    ctx.save(); ctx.translate(nd.x,nd.y); ctx.rotate(ang);
    ctx.fillStyle='rgba(30,70,18,0.38)';
    ctx.beginPath(); ctx.moveTo(r*0.5,0); ctx.lineTo(0,r*0.42); ctx.lineTo(-r*0.5,0); ctx.lineTo(0,-r*0.42); ctx.closePath(); ctx.fill();
    ctx.restore();
    const fl=Math.max(0,1-Math.abs(k-wave)/2.2);
    if(fl>0){ ctx.beginPath(); ctx.arc(nd.x,nd.y,r,0,Math.PI*2); ctx.fillStyle='rgba(255,255,210,'+(fl*0.85).toFixed(2)+')'; ctx.fill(); }
  }
  // segments that snapped off: shrink, fade, drift
  s.drops=s.drops.filter(d=>T-d.born<0.6);
  for(const d of s.drops){
    const u=(T-d.born)/0.6;
    ctx.globalAlpha=1-u;
    ctx.beginPath(); ctx.arc(d.x+Math.cos(d.ang)*u*16,d.y+Math.sin(d.ang)*u*16-u*8,d.r*(1-0.5*u),0,Math.PI*2);
    ctx.fillStyle=darken(base,0.15); ctx.fill(); ctx.lineWidth=1.3; ctx.strokeStyle='rgba(28,52,14,0.7)'; ctx.stroke();
  }
  ctx.restore();
}

function snakeTongue(ctx,R,amt,wig){
  if(amt<=0.03) return;
  const L=R*1.05*amt, fork=R*0.38*amt, sp=0.5;
  ctx.save();
  ctx.translate(R*0.86,0); ctx.rotate(wig);
  ctx.lineCap='round'; ctx.lineJoin='round';
  const path=()=>{
    ctx.beginPath(); ctx.moveTo(0,0); ctx.lineTo(L,0);
    ctx.moveTo(L,0); ctx.lineTo(L+Math.cos(sp)*fork,Math.sin(sp)*fork);
    ctx.moveTo(L,0); ctx.lineTo(L+Math.cos(sp)*fork,-Math.sin(sp)*fork);
  };
  ctx.strokeStyle='#6d0f24'; ctx.lineWidth=R*0.21; path(); ctx.stroke();
  ctx.strokeStyle='#ff3f5e'; ctx.lineWidth=R*0.11; path(); ctx.stroke();
  ctx.restore();
}

function snakeDrawHead(b,g,ctx,info){
  const s=b.state.snk, T=g.fxT, R=b.radius, base=b.def.color;
  const chomp=clamp(1-(T-s.chompAt)/0.24,0,1);
  const pop=(info.pop||1)*(1+0.12*Math.sin(chomp*Math.PI));
  ctx.save();
  // grounded shadow (not scaled)
  ctx.beginPath(); ctx.ellipse(b.x,b.y+R*0.72,R*0.8,R*0.28,0,0,Math.PI*2);
  ctx.fillStyle='rgba(0,0,0,0.14)'; ctx.fill();
  ctx.translate(b.x,b.y); ctx.scale(pop,pop);
  const glow=14+(info.lowHp?10*(0.6+0.4*Math.sin(g.t*8)):0)+(info.flashT||0)*14;
  ctx.shadowColor=info.lowHp?'#ff3b3b':b.color; ctx.shadowBlur=glow;
  ctx.beginPath(); ctx.arc(0,0,R,0,Math.PI*2);
  const gr=ctx.createRadialGradient(-R*0.35,-R*0.4,R*0.15,0,0,R*1.05);
  gr.addColorStop(0,lighten(base,0.55)); gr.addColorStop(0.55,base); gr.addColorStop(1,darken(base,0.22));
  ctx.fillStyle=gr; ctx.fill();
  ctx.shadowBlur=0;
  ctx.lineWidth=3.5; ctx.strokeStyle=b.color; ctx.stroke();
  ctx.lineWidth=1; ctx.strokeStyle='rgba(26,26,26,0.55)'; ctx.stroke();

  // everything on the face is drawn relative to the heading (+x = forward)
  ctx.save();
  ctx.beginPath(); ctx.arc(0,0,R-1,0,Math.PI*2); ctx.clip();
  ctx.rotate(s.face);
  // dorsal stripe + scale pattern
  ctx.fillStyle='rgba(30,80,20,0.30)';
  ctx.beginPath(); ctx.ellipse(-R*0.28,0,R*0.8,R*0.2,0,0,Math.PI*2); ctx.fill();
  ctx.strokeStyle='rgba(22,60,12,0.22)'; ctx.lineWidth=1;
  for(let row=-2;row<=2;row++) for(let col=-2;col<=1;col++){
    const cx=col*R*0.42+(row%2?R*0.21:0)-R*0.15, cy=row*R*0.36;
    ctx.beginPath(); ctx.arc(cx,cy,R*0.2,0.2,Math.PI-0.2); ctx.stroke();
  }
  // lighter snout
  ctx.fillStyle='rgba(255,255,215,0.20)';
  ctx.beginPath(); ctx.ellipse(R*0.5,0,R*0.5,R*0.55,0,0,Math.PI*2); ctx.fill();
  ctx.restore();

  ctx.save();
  ctx.rotate(s.face);
  // eyes: big, yellow, slit pupils, slanted brows; they blink every few seconds
  const blink=((T+s.blinkSeed)%3.6)<0.12;
  for(const sd of [-1,1]){
    const ex=R*0.30, ey=sd*R*0.5, er=R*0.27;
    ctx.beginPath(); ctx.arc(ex,ey,er,0,Math.PI*2);
    ctx.fillStyle='#fffdf0'; ctx.fill();
    ctx.lineWidth=1.2; ctx.strokeStyle='#1c2e0e'; ctx.stroke();
    if(blink){
      ctx.beginPath(); ctx.arc(ex,ey,er,0,Math.PI*2); ctx.fillStyle=darken(base,0.1); ctx.fill(); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(ex-er*0.2,ey-er*0.8); ctx.lineTo(ex-er*0.2,ey+er*0.8); ctx.stroke();
    } else {
      ctx.beginPath(); ctx.arc(ex+R*0.03,ey,er*0.72,0,Math.PI*2);
      ctx.fillStyle='#e6ee4a'; ctx.fill();
      ctx.fillStyle='#10180a';
      ctx.beginPath(); ctx.ellipse(ex+R*0.03,ey,er*0.2,er*0.62,0,0,Math.PI*2); ctx.fill();
      ctx.fillStyle='rgba(255,255,255,0.9)';
      ctx.beginPath(); ctx.arc(ex-er*0.15,ey-sd*er*0.3,er*0.16,0,Math.PI*2); ctx.fill();
    }
    ctx.strokeStyle='rgba(22,50,10,0.85)'; ctx.lineWidth=R*0.09; ctx.lineCap='round';
    ctx.beginPath(); ctx.moveTo(ex-er*0.55,ey-sd*er*1.12); ctx.lineTo(ex+er*0.85,ey-sd*er*0.62); ctx.stroke();
  }
  // nostrils + a hint of a mouth line
  ctx.fillStyle='rgba(20,40,10,0.8)';
  for(const sd of [-1,1]){ ctx.beginPath(); ctx.ellipse(R*0.74,sd*R*0.13,R*0.045,R*0.03,0,0,Math.PI*2); ctx.fill(); }
  ctx.strokeStyle='rgba(20,40,10,0.55)'; ctx.lineWidth=1.2;
  ctx.beginPath(); ctx.arc(R*0.15,0,R*0.75,-0.32,0.32); ctx.stroke();
  // tongue flick
  const t=T-s.tongueAt, per=0.34, dur=per*s.tongueFlicks;
  const amt=(t>=0&&t<=dur)?Math.pow(Math.sin(Math.PI*((t%per)/per)),0.7):0;
  snakeTongue(ctx,R,amt,Math.sin(T*46)*0.26*amt);
  ctx.restore();

  if((info.flashT||0)>0){
    ctx.beginPath(); ctx.arc(0,0,R,0,Math.PI*2);
    ctx.fillStyle='rgba(255,255,255,'+(info.flashT*0.75).toFixed(2)+')'; ctx.fill();
  }
  ctx.restore();
}

// length chip under the head: shows the number of segments, red while the tail is too short to lash
function snakeDrawChip(b,g,ctx){
  const s=b.state.snk, y=b.y+b.radius+13, ok=s.segs>=SNAKE.MIN_LASH;
  ctx.save();
  ctx.beginPath(); ctx.arc(b.x,y,8.5,0,Math.PI*2);
  ctx.fillStyle='rgba(255,255,255,0.9)'; ctx.fill();
  ctx.lineWidth=1.2; ctx.strokeStyle=ok?'#3f7d1a':'#c0392b'; ctx.stroke();
  ctx.font='bold 10px sans-serif'; ctx.textAlign='center'; ctx.textBaseline='middle';
  ctx.fillStyle=ok?'#2f5f12':'#c0392b';
  ctx.fillText(String(s.segs),b.x,y+0.5);
  ctx.restore();
}

function snakeDraw(b,g,ctx,info){
  const s=b.state.snk; if(!s) return;
  snakeFollow(b,g);
  for(const a of s.apples) snakeDrawApple(ctx,g,a);
  snakeDrawTail(b,g,ctx);
  snakeDrawHead(b,g,ctx,info);
  snakeDrawChip(b,g,ctx);
}

// ---- Werewolf Ball form cycle -------------------------------------------
const WEREWOLF_HUMAN_SECONDS=13;   // seconds spent in HUMAN form before turning into a wolf, in game time
const WEREWOLF_WOLF_SECONDS=10;    // seconds the WOLF form lasts before turning back, in game time
