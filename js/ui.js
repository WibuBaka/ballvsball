...
function submitSecretCode(){
  ...
  if(code==='FLO'){
    ...
    if(subaruUnlocked){
      showSecretMsg('Return by Death: chế độ SUBARU đã mở rồi.','#8fdcff');
    } else {
      subaruUnlocked=true;
      showSecretMsg('🔁 SUBARU! Sau khi Return by Death chết sẽ có ô nhập số lần chết.','#b58cff');
    }
  } else if(code==='ANXIETY'){
    // The Weirdcore?!: bật/tắt bảng debug (nút WC / phím 1-6) khi có The Weirdcore trong trận
    const on=(typeof Weirdcore!=='undefined') ? Weirdcore.setDebugUnlocked(!WEIRDCORE_DEBUG) : false;
    showSecretMsg(on ? '👁️ ANXIETY: đã mở bảng WC (nút WC khi chơi The Weirdcore?!).' : '👁️ ANXIETY: đã tắt bảng WC.', '#c9c9d4');
...
document.getElementById('mapNext').onclick=()=>{ buildBallGrids(); setPickPanel(1); showScreen('ball'); revealChosenPicks(); };
...
document.getElementById('btnMenu').onclick=()=>{ stopMatch(); resetReturnDeathRuns(); hideSubaruPanel(); showScreen('menu'); };
document.getElementById('btnRematch').onclick=()=>{ startMatch(true); };
...
function buildGallery(){
  ...
  if(typeof Weirdcore!=='undefined') Weirdcore.decorateUi();
}
...
/* Scroll a picker grid (only its own scroll area, never the page) so the chosen
   ball is visible - e.g. the ball the AI rolled at random can sit several rows
   down. No-op while the screen is hidden (nothing to measure yet). */
function revealChosen(host){
  const el=host && host.querySelector('.pick-item.chosen');
  if(!el || !host.clientHeight) return;
  const h=host.getBoundingClientRect(), c=el.getBoundingClientRect();
  if(c.top>=h.top && c.bottom<=h.bottom) return;
  host.scrollTop += (c.top-h.top) - (host.clientHeight-c.height)/2;
}
function revealChosenPicks(){
  ['p1Grid','p2Grid'].forEach(id=>revealChosen(document.getElementById(id)));
}
function updateSummary(player, bt){
  ...
  el.innerHTML=`<b>${bt.icon} ${bt.name}</b>
    ...
    <div class="stat-desc" style="margin-top:6px;color:var(--text-1)">${bt.descSimple}</div>`;
  refreshPickSwitch();
  if(typeof Weirdcore!=='undefined') Weirdcore.decorateUi();
}
/* ---- phone layout: one panel at a time (P1 / P2) ---- */
function setPickPanel(n){
  document.getElementById('p1Panel').classList.toggle('mobile-active',n===1);
  document.getElementById('p2Panel').classList.toggle('mobile-active',n===2);
  document.getElementById('pickSwitch1').classList.toggle('active',n===1);
  document.getElementById('pickSwitch2').classList.toggle('active',n===2);
  revealChosen(document.getElementById(n===1?'p1Grid':'p2Grid')); // grid was hidden -> measure now
}
function refreshPickSwitch(){
  const find=id=>BALL_TYPES.find(b=>b.id===id);
  const b1=find(p1Ball), b2=find(p2Ball);
  const s1=document.getElementById('pickSwitch1'), s2=document.getElementById('pickSwitch2');
  if(!s1||!s2) return;
  s1.querySelector('.ps-name').textContent='PLAYER 1';
  s1.querySelector('.ps-ball').textContent=b1?b1.icon:'❔';
  s2.querySelector('.ps-name').textContent=vsAI?'🤖 ĐỐI THỦ (AI)':'PLAYER 2';
  s2.querySelector('.ps-ball').textContent=b2?b2.icon:'❔';
}
document.getElementById('pickSwitch1').onclick=()=>setPickPanel(1);
document.getElementById('pickSwitch2').onclick=()=>setPickPanel(2);
