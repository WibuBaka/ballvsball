...
class GameEngine{
  constructor(canvas){
    ...
    this.muzzleFlashes=[]; // brief flash at the barrel/hand when a projectile is fired
    this.mapDef=null;
    this.mapObstacles=[];
    this.mapPortals=[];
  ...
  start(){
    ...
    this.winner=null;
    this.aiFireAt=null;
    this.floatTexts=[]; this.particles=[]; this.projectiles=[]; this.hazards=[]; this.bombs=[]; this.trains=[]; this.cellClones=[]; this.explosions=[]; this.tornadoes=[]; this.muzzleFlashes=[];
    ...
    // THE WEIRDCORE?!: set up screen effects / audio / name glitch if this match contains it
    if(typeof Weirdcore!=='undefined') Weirdcore.onMatchStart(this);
  }
  stop(){ this.running=false; if(typeof Weirdcore!=='undefined') Weirdcore.onMatchStop(this); }
  loop(ts){
  ...
  _updateWorld(dt){
    ...
    for(const ball of this.balls){
      ...
      // THE WEIRDCORE?!: anxiety stretches the ball's own skill timers (state.anxCdMult >= 1)
      if(ball.def.update && powerOk(ball,this)) ball.def.update(ball,localDt/(ball.state.anxCdMult||1),this);
      ...
      // THE WEIRDCORE?!: anxiety steering (rotates velocity only), skill-cooldown drag, frame stutter
      localDt=Weirdcore.movementHook(ball,localDt,dt,this);
      clampBallSpeed(ball);
      ...
      Weirdcore.clampArena(ball,this);
    }
    ...
    this.muzzleFlashes=this.muzzleFlashes.filter(m=>this.t-m.start<m.dur);
  ...
  _postStep(hpBefore){
    ...
    this.balls.forEach((ball,i)=>{
      ...
      const burnTick=ball.state.wcBurn; ball.state.wcBurn=false; // Weirdcore burn tick: flash only, no shake/sparks
      let dmg=hpBefore[i]-ball.hp;
      // THE WEIRDCORE?!: an anxious ball deals less damage. Every damage source funnels through this
      // HP diff, so scaling here covers collisions, projectiles, hazards, skills... (state.anxDmgMult
      // lives on the ANXIOUS ball and is >= 0.55). Lethal hits stay lethal, same rule as dmgResist.
      if(dmg>0.01 && ball.hp>0){
        const foe=this.opponentOf(ball), am=foe&&foe.state.anxDmgMult;
        if(am&&am<1){ ball.hp=hpBefore[i]-dmg*am; dmg=hpBefore[i]-ball.hp; }
      }
      // Return by Death damage resistance (state.dmgResist = 1.05^stacks): every
      // damage source funnels through this HP diff, so damage taken is divided
      // here - it can only shrink a hit, never push it negative. A hit that
      // already emptied the bar (hp<=0) stays lethal.
      if(dmg>0.01 && ball.hp>0 && ball.state.dmgResist>1){
        ball.hp=hpBefore[i]-dmg/ball.state.dmgResist;
        dmg=hpBefore[i]-ball.hp;
      }
      if(dmg>0.01){
    ...
    for(const ball of this.balls){
      if(ball.hp<=0 && ball.alive){
        ...
        if(!revived){
          ball.alive=false;
          if(ball.type==='returnbydeath') markReturnDeath(ball);
          this.endMatch(this.opponentOf(ball));
        }
      }
    }
    this.updateHUD();
    Weirdcore.afterHUD(this);
  }
  ...
  applyProjectileDamage(pr,target){
    if(skillDodge(target,this)) return; // Return by Death: the whole projectile hit is avoided
    ...
    for(const hz of this.hazards){
      for(const ball of this.balls){
        ...
        if(pointInHazard(hz,ball)){
          ...
          if(hz.type==='frostzone' && !(hz.dodged && hz.dodged[ball.player])) inFrostThisFrame.add(ball);
        }
      }
    }
    for(const ball of this.balls){
  ...
  updateBombs(){
    for(const bomb of this.bombs){
      if(this.t>=bomb.at && !bomb.exploded){
        bomb.exploded=true;
        explodeAt(this,bomb.x,bomb.y,bomb.owner,155,dmgFor(bomb.owner,6));
      }
    }
  ...
  updateTrains(dt){
    for(const tr of this.trains){
      ...
      for(const other of this.balls){
        ...
        if(hit){
          tr.hitSet.add(other);
          if(skillDodge(other,this)) continue;
  ...
  spawnExplosionRing(x,y,radius,fillRGB,strokeRGB){
    const rayCount=8+Math.floor(rand(0,5));
    const rays=[];
    for(let i=0;i<rayCount;i++) rays.push(Math.PI*2*i/rayCount + rand(-0.15,0.15));
    this.explosions.push({x,y,radius,start:this.t,dur:0.45,fillRGB,strokeRGB,rays});
    this.triggerShake(clamp(radius*0.045,3,13), 0.22);
    // smoke puffs that linger and drift a little after the flash/ring fade
    spawnParticles(this,x,y,5,{color:'rgba(90,90,90,0.5)',type:'smoke',speed:35,life:1.1,size:radius*0.28,gravity:-14});
    // chunky debris chips flung outward, tumbling as they fly
    spawnParticles(this,x,y,7,{color:strokeRGB?`rgb(${strokeRGB})`:'#7a5a3a',type:'debris',speed:radius*1.1,life:0.55,size:5,gravity:180});
  }
  ...
  updateTornadoes(dt){
    for(const tor of this.tornadoes){
      ...
      if(!tor.captured){
        if(d<tor.pullRadius){
          // Return by Death: one roll per tornado (first contact); dodged = it ignores the victim
          if(tor.dodged===undefined) tor.dodged=skillDodge(target,this);
          if(tor.dodged) continue;
          tor.exposure+=dt;
  ...
  endMatch(winnerBall){
    ...
    renderSubaruPanel(); // SUBARU code: add-deaths panel if a Return by Death ball just died
    this.updateTurnGlow();
  ...
  render(dt){
    ...
    for(const hz of this.hazards){
      ...
      // spawn pop: fades the hazard in over its first ~0.3s instead of
      // appearing at full strength instantly, plus a brief white bloom flash
      const spawnAge = hz.bornAt!=null ? this.t-hz.bornAt : 999;
      const POP_DUR=0.3;
      const popT = clamp(spawnAge/POP_DUR,0,1);
      ctx.globalAlpha = 0.15+0.85*popT;
      ...
      // bloom flash burst at spawn - a bright expanding ring on top of
      // everything so traps/zones/beams read as a quick "bùng lên" flare
      // instead of appearing abruptly (poison/frost/web/laser/zone hazards)
      if(hz.bornAt!=null && popT<1 && hz.type!=='leafpatch'){
        const bcx = hz.rectType ? hz.x+hz.w/2 : hz.x;
        const bcy = hz.rectType ? hz.y+hz.h/2 : hz.y;
        const baseR = hz.rectType ? Math.max(hz.w,hz.h)/2 : (hz.r||30);
        const flashR = baseR*(0.35+0.9*popT);
        ctx.save();
        ctx.globalAlpha=(1-popT)*0.85;
        ctx.beginPath(); ctx.arc(bcx,bcy,flashR,0,Math.PI*2);
        ctx.fillStyle='rgba(255,255,255,0.85)'; ctx.fill();
        ctx.strokeStyle='rgba(255,255,255,0.9)'; ctx.lineWidth=3;
        ctx.beginPath(); ctx.arc(bcx,bcy,flashR*1.35,0,Math.PI*2); ctx.stroke();
        ctx.restore();
      }
      ctx.restore();
    }
    // muzzle flashes (Machine Gun/Thief/Bow etc.) - bright cone flare at the
    // firing point, oriented toward the shot, gone within a couple frames
    for(const mf of this.muzzleFlashes){
      const p=clamp((this.t-mf.start)/mf.dur,0,1);
      const a=1-p;
      ctx.save();
      ctx.translate(mf.x,mf.y); ctx.rotate(mf.ang);
      ctx.globalAlpha=a;
      ctx.shadowColor=mf.color; ctx.shadowBlur=14;
      const len=mf.size*(3.2+2*p);
      const grd=ctx.createLinearGradient(0,0,len,0);
      grd.addColorStop(0,'rgba(255,255,255,0.95)');
      grd.addColorStop(0.4,mf.color);
      grd.addColorStop(1,'rgba(255,255,255,0)');
      ctx.beginPath();
      ctx.moveTo(0,0);
      ctx.lineTo(len,-mf.size*0.55);
      ctx.lineTo(len,mf.size*0.55);
      ctx.closePath();
      ctx.fillStyle=grd; ctx.fill();
      ctx.beginPath(); ctx.arc(0,0,mf.size*0.9,0,Math.PI*2);
    ...
    for(const ex of this.explosions){
      ...
      // radiating rays/spikes shooting out from the hot core, brightest and
      // longest right at ignition, shrinking back in as the ring expands
      if(ex.rays && p<0.6){
        const rp=clamp(p/0.6,0,1);
        const rayLen=ex.radius*(0.95-0.65*rp);
        ctx.save();
        ctx.globalAlpha=(1-rp)*0.85;
        ctx.strokeStyle=`rgb(${strokeRGB})`;
        ctx.lineCap='round';
        for(const a of ex.rays){
          const innerR=6+ex.radius*0.08;
          ctx.lineWidth=3*(1-rp)+1;
          ctx.beginPath();
          ctx.moveTo(ex.x+Math.cos(a)*innerR, ex.y+Math.sin(a)*innerR);
          ctx.lineTo(ex.x+Math.cos(a)*(innerR+rayLen), ex.y+Math.sin(a)*(innerR+rayLen));
          ctx.stroke();
        }
        ctx.restore();
      }
    ...
    for(const ball of this.balls){
      ...
      // THE WEIRDCORE?!: shake / afterimage of an anxious ball (draw-only, physics untouched)
      const _wcSaved=Weirdcore.beginBall(ball,this,ctx);

      if(ball.def.drawBody){
        // custom body (The Weirdcore?! void ball) - replaces shadow/body/icon below
        ball.def.drawBody(ball,this,ctx,{pop, flashT:clamp((ball.state.flashUntil||0)-this.fxT,0,0.14)/0.14, lowHp});
      } else {
      ctx.save();
    ...
    for(const p of this.particles){
      ...
      if(p.type==='glow'){
        ...
        // slow-drifting, growing, softly-fading puff - lingers after the flash
        const age=this.fxT-p.born;
        const grow=1+age*1.6;
        ctx.globalAlpha=a*0.35;
        ctx.filter='blur(1px)';
        ctx.beginPath(); ctx.arc(p.x,p.y,p.r*grow,0,Math.PI*2);
        ctx.fillStyle=p.color; ctx.fill();
        ctx.filter='none';
      } else if(p.type==='debris'){
        // chunky tumbling chip flung out by an explosion/impact
        p.rot=(p.rot||0)+(p.rotSpeed||(p.rotSpeed=rand(-9,9)))*dt;
        ctx.globalAlpha=a;
        ctx.translate(p.x,p.y); ctx.rotate(p.rot);
        ctx.fillStyle=p.color;
        ctx.fillRect(-p.r*0.55,-p.r*0.4,p.r*1.1,p.r*0.8);
      } else {
    ...
    // THE WEIRDCORE?!: frame-tearing / RGB split on this canvas + fullscreen overlay
    Weirdcore.postRender(this,dt);
  }
}
