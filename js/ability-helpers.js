...
function spawnFloatText(g,x,y,text,color){
  const numeric=/^-([0-9]+(?:\.[0-9]+)?)(?:% (.+))?$/.exec(text);
  const key=numeric ? (numeric[2] ? `num:${numeric[2]}` : 'num:generic') : `txt:${text}`;
  const now=g.t;
  const existing=g.floatTexts.slice().reverse().find(f=>f.key===key && now-f.born<0.2 && dist(f.x,f.y,x,y)<42);
  if(existing && numeric){
    const amount=(existing.amount||parseFloat(numeric[1])||0)+(parseFloat(numeric[1])||0);
    existing.amount=amount;
    existing.text=`-${amount.toFixed(amount%1?1:0)}${numeric[2]?`% ${numeric[2]}`:''}`;
    existing.x=x+rand(-8,8);
    existing.y=y+rand(-5,5);
    existing.born=now;
    existing.until=now+1.05;
    existing.drift=rand(-14,14);
    return;
  }
  g.floatTexts.push({x:x+rand(-9,9),y:y+rand(-5,5),startX:x,text,color,born:now,until:now+1.05,drift:rand(-14,14),key,amount:numeric?parseFloat(numeric[1]):null});
}
