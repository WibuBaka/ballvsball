// Return by Death Ball: `state.dodgeChance` (0..1, +3 percentage points per stack)
// is the chance to avoid ONE special-skill hit outright. Call this right before a
// skill lands on `target`; true = dodged, so the caller skips the WHOLE hit (damage,
// status, knockback). Never used for plain ball-vs-ball collision damage.
function skillDodge(target,g){
  const c=target && target.state.dodgeChance;
  if(!(c>0) || Math.random()>=c) return false;
  spawnFloatText(g,target.x,target.y-30,'DODGE!','#7dd3fc');
  spawnParticles(g,target.x,target.y,8,{color:'#7dd3fc',type:'glow',speed:120,life:0.35});
  return true;
}
// per-frame zones (Frost trail) can't roll every frame: roll once per patch per victim
function hazardDodged(hz,ball,g){
  const d=hz.dodged||(hz.dodged={});
  if(d[ball.player]===undefined) d[ball.player]=skillDodge(ball,g);
  return d[ball.player];
}
function florentinoStrike(b,g,other){
  if(skillDodge(other,g)){ florentinoEnd(b,g); return; } // dodged: the whole dash/dance combo fizzles
...
function fireProjectile(g,owner,opts){
  ...
  // Muzzle flash: a quick bright flare + a few sparks kicked out in the
  // firing direction, so a shot reads as launched from something rather
  // than just appearing mid-air (Machine Gun/Thief/Bow and similar).
  const fx=opts.x!=null?opts.x:owner.x, fy=opts.y!=null?opts.y:owner.y;
  const ang=Math.atan2(opts.vy||0,opts.vx||0);
  const flashColor=opts.color||'#fff';
  g.muzzleFlashes.push({x:fx,y:fy,ang,color:flashColor,start:g.t,dur:0.14,size:(opts.r||6)});
  spawnParticles(g,fx,fy,5,{color:flashColor,type:'spark',speed:150,life:0.18});
}
...
function spawnHazard(g,hz){
  ...
  hz.bornAt = hz.bornAt!=null ? hz.bornAt : g.t;
  g.hazards.push(hz);
  // "bùng lên" spawn burst so traps/zones/beams don't just pop into
  // existence silently - a quick colored glow puff at the moment of birth
  const cx = hz.rectType ? hz.x+hz.w/2 : hz.x;
  const cy = hz.rectType ? hz.y+hz.h/2 : hz.y;
  const burstColor={poisontrap:'#8dff7a',frostzone:'#a8e8ff',web:'#cfcfcf',laser:'#ff8a8a',zonebox:'#ff6a6a',leafpatch:'#8fd95f'}[hz.type]||'#ffffff';
  spawnParticles(g,cx,cy,10,{color:burstColor,type:'glow',speed:130,life:0.35});
}
...
function applyHazardEffect(hz,ball,g,dt){
  ...
  switch(hz.type){
    case 'poisontrap':
      ...
      break;
    case 'frostzone':
      if(hazardDodged(hz,ball,g)) break;
