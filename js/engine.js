class GameEngine{
  ...
  _bindInput(){
    ...
    const up=(e)=>{
      ...
      if(this.phase==='aim1'){
        if(this.vsAI){
          this.phase='aim2';
          this.setStatus('🤖 AI đang ngắm...', 'turn-p2');
          ...
          this.phase='aim2';
          this.setStatus('Kéo bóng để ngắm — PLAYER 2', 'turn-p2');
        }
      } else if(this.phase==='aim2'){
        this.phase='sim';
        this.setStatus('⚔️ ĐANG GIAO TRANH ⚔️', 'battle');
      }
    };
  ...
  setStatus(text, tone){
    ...
    el.textContent=text;
    el.classList.remove('aiming','turn-p1','turn-p2','battle');
    if(tone) el.classList.add(tone);
  }
  syncAimHud(){
    const el=document.getElementById('statusBar');
    const aiming=(this.phase==='aim1'||this.phase==='aim2');
    const power=aiming && this.dragging && this.dragStart && this.dragCurrent
      ? clamp(Math.hypot(this.dragStart.x-this.dragCurrent.x,this.dragStart.y-this.dragCurrent.y)/160,0,1) : 0;
    el.classList.toggle('aiming',aiming);
    el.style.setProperty('--aim-power',`${Math.round(power*100)}%`);
    el.dataset.power=String(Math.round(power*100));
    if(!aiming){ el.classList.remove('turn-p1','turn-p2','battle'); }
    else {
      el.classList.remove('turn-p1','turn-p2');
      el.classList.add(this.phase==='aim1'?'turn-p1':'turn-p2');
    }
    this._aimPower=power;
  }
  syncTurnHud(){
    const active=this.phase==='aim1'?1:this.phase==='aim2'?2:0;
    document.querySelector('.hp-p1')?.classList.toggle('turn-active',active===1);
    document.querySelector('.hp-p2')?.classList.toggle('turn-active',active===2);
  }
  start(){
    this.running=true;
    this.t=0;
    ...
    this.shakeMag=0; this.shakeUntil=0; this._lastHudHp=null; this._lastHudMom=null; this._aimPower=0;
    this.setStatus('Kéo bóng để ngắm — PLAYER 1', 'turn-p1');
  ...
  updateHUD(){
    ...
    [[a,'p1'],[b,'p2']].forEach(([ball,pfx])=>{
      ...
      const typeIcon=document.getElementById(pfx+'HpTypeIcon');
      if(typeIcon){
        typeIcon.textContent=isMom?'🌀':'❤';
        typeIcon.classList.toggle('momentum',isMom);
      }
  ...
  endMatch(winnerBall){
    ...
    const winnerColor=winnerBall.player===1?'#d94f4f':'#3d6fd9';
    msg.style.display='block';
    msg.style.color=winnerColor;
    msg.style.borderColor=winnerColor;
    msg.innerHTML=`<div class=\"round-win-inner\"><span class=\"round-winner-ball\">${winnerBall.def.icon}</span><span>🏆 PLAYER ${winnerBall.player} THẮNG!</span></div><div class=\"round-win-sub\">${winnerBall.def.name} quyết định trận đấu</div>`;
    const confettiColors=[winnerColor,'#2fd4ff','#ffd15c','#8dff7a','#e0568a','#c9a8ff'];
    for(let i=0;i<26;i++){
      const c=document.createElement('span');
      c.className='round-confetti';
      c.style.setProperty('--dx',`${rand(-190,190).toFixed(0)}px`);
      c.style.setProperty('--dy',`${rand(-130,150).toFixed(0)}px`);
      c.style.setProperty('--rot',`${rand(-420,420).toFixed(0)}deg`);
      c.style.background=confettiColors[i%confettiColors.length];
      c.style.animationDelay=`${rand(0,0.12).toFixed(2)}s`;
      msg.appendChild(c);
    }
    spawnParticles(this,winnerBall.x,winnerBall.y,28,{color:winnerColor,speed:250,type:'spark',life:0.9,size:4});
    spawnParticles(this,winnerBall.x,winnerBall.y,14,{color:'#ffd15c',speed:160,type:'glow',life:0.9,size:5});
    this.syncTurnHud();
  ...
  render(){
    ...
    for(const ball of this.balls){
      ...
      for(let i=ball.state.trail.length-1;i>=0;i--){
        ...
        ctx.fillStyle=ball.def.color||ball.color; ctx.fill();
        ctx.restore();
      }
      ...
      const bodyColor=ball.def.color||ball.color;
