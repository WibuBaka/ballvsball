  ...
  groupsToShow.forEach(gKey=>{
    ...
    BALL_TYPES.filter(bt=>bt.group===gKey).forEach(bt=>{
      ...
      card.innerHTML=`<div class="ball-dot" style="background:${meta.color};color:${meta.color}"></div>
        <div class="info">
          <b>${bt.icon} ${bt.name}</b>
          <div class="stat-row" style="margin:3px 0;font-size:.72rem;font-weight:700;color:var(--text-1);"><span>${bt.momentumBar?'ĐL 100%':'HP '+bt.hp}</span><span>SPD ${bt.speed}</span><span>DMG ${bt.dmg}</span></div>
          <span>${text}</span>
        </div>`;
...
function updateSummary(player, bt){
  ...
  el.innerHTML=`<b>${bt.icon} ${bt.name}</b>
    <div class="stat-row"><span>HP</span><span>${bt.hp}</span></div>
